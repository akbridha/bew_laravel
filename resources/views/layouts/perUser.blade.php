...
<!-- <H2 mb-7>tes var dump {{$siaran}} </H2> -->
  ...
  <article class = 'mb-5 border-bottom'>
    ...
    <p>By <a href="/authors/{{$siaran->user->id}}">{{$siaran->user->name}}</a> in <a href = '/Kategori/{{$siaran->kategori->slug}}'class='text-decoration-none'>{{$siaran->kategori->name}}</a>
    

    <h5>{{ $siaran->user->name}}</h5>
    <p>{{ $siaran->excerpt}}</p>
